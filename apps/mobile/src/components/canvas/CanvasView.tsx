import type { CanvasData, CanvasEdge as CanvasEdgeType, CanvasEdgeLineStyle, CanvasNode, CanvasNodeSide } from '@nodeira/shared-types';
...
export interface CanvasViewHandle {
  addNode: (type: AddNodeType, canvasX: number, canvasY: number, extraData?: Record<string, unknown>) => void;
  fitView: () => void;
}
...
function computeFitView(
  ...
  const padding = 40;
  const sc = Math.min(
    (screenWidth - padding * 2) / bw,
    (screenHeight - padding * 2) / bh,
    2,
  );
  ...
  function CanvasView({ initialData, onChange, readOnly = false }, ref) {
    ...
    const pinchGesture = Gesture.Pinch()
      ...
      .onUpdate((e) => {
        ...
        translateX.value = e.focalX - ((pinchFocalX.value - half - pinchSavedTX.value) / savedScale.value) * newScale - half;
        translateY.value = e.focalY - ((pinchFocalY.value - half - pinchSavedTY.value) / savedScale.value) * newScale - half;
      });
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text style={{ fontSize: 13, fontWeight: '600', color: '#868e96', paddingHorizontal: 20, paddingBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                Line Style
              </Text>
              {LINE_STYLES.map((s) => {
                const currentStyle = edges.find((e) => e.id === selectedEdgeId)?.lineStyle ?? 'bezier';
                return (
                  <Pressable
                    key={s}
              ...
              <Pressable
                ...
                style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 14, gap: 12, marginTop: 8, borderTopWidth: 1, borderTopColor: border }}
              >
