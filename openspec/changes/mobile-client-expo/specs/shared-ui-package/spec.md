...
A new pnpm workspace `packages/ui` SHALL be created. It SHALL be a Vite library build targeting ESM. `apps/web` and `apps/desktop` SHALL both declare it as a workspace dependency and import shared components from it. `apps/mobile` SHALL NOT depend on `packages/ui`; the mobile app uses its own React Native UI layer.
...
#### Scenario: Mobile app does not import from shared package

- **WHEN** `apps/mobile` is built
- **THEN** no import from `@nodeira/ui` exists in the mobile bundle

---

### Requirement: Shared package exports presentational components only

`packages/ui` SHALL export only components that are pure presentational or editor-layer concerns: `NoteEditor`, `NoteList`, `Sidebar`, and shared layout primitives. Route-level components, TanStack Router bindings, and app-level providers SHALL remain in their respective app packages. React Native components SHALL NOT be added to `packages/ui`; they belong in `apps/mobile`.
